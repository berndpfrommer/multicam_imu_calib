...
def to_camerainfo(k, camera_input_name, camera_output_name):
    ...
    intrinsics = np.array([[intr['fx'], 0, intr['cx']],[0, intr['fy'], intr['cy']],[0,0,1]])
    ...
    y['image_width']  = 1920
    y['image_height'] = 1024
    y['camera_name']  = k['image_topic'] if camera_input_name is None else camera_input_name
...
def main():
    ...
    parser.add_argument("-i", "--input", required=True, help="input file in multicam_imu format")
    parser.add_argument("-o", "--output", required=True, help="output file in opencv/camerainfo format")
    parser.add_argument("-c", "--camera", default="camera_0", help="name of camera to extract")
