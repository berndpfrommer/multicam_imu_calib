...
static cv::Mat draw_msg(const cv::Mat & img, const TargetArray & msg)
{
  cv::Mat colorImg;
  cv::cvtColor(img, colorImg, cv::COLOR_GRAY2BGR);
  for (const auto & targ : msg.targets) {
    for (const auto & marker : targ.markers) {
      if (marker.type == "apriltag") {
        const auto & c = marker.image_points;
        cv::Point2d cent{
        ...
        const std::array<std::array<double, 2>, 4> corners{
          {{c[0].x, c[0].y},
           ...
           {c[2].x, c[2].y},
           {c[3].x, c[3].y}}};
        draw(colorImg, std::stoi(marker.id), {cent.x, cent.y}, corners);
      }
    }
  }
  return (colorImg);
...
: Node("detection_draw", options)
{
  ...
  subscription_check_timer_ = rclcpp::create_timer(
    ...
    std::bind(&DetectionDraw::subscriptionCheckTimerExpired, this));
#endif
}
...
void DetectionDraw::subscribe()
{
  RCLCPP_INFO(this->get_logger(), "subscribing to tags and image!");
  tag_sub_ = create_subscription<TargetArray>(
    "tags", rclcpp::QoS(10),
...
void DetectionDraw::unsubscribe()
{
  tag_sub_.reset();
  image_sub_->shutdown();
...
void DetectionDraw::subscriptionCheckTimerExpired()
{
...
void DetectionDraw::processFrame(
  const TargetArray::ConstSharedPtr & tags, const Image::ConstSharedPtr & msg)
{
  if (image_pub_.getNumSubscribers() != 0) {
...
void DetectionDraw::imageCallback(const Image::ConstSharedPtr & img)
{
...
void DetectionDraw::tagCallback(const TargetArray::ConstSharedPtr & tags)
{
...
void DetectionDraw::processBuffers()
{
  ...
  while (!tag_buffer_.empty()) {
    ...
    const auto tag_time = get_time<TargetArray>(tags);
    // purge old images
    ...
    while (!tag_buffer_.empty() &&
           get_time<TargetArray>(*(tag_buffer_.begin())) < img_time) {
      tag_buffer_.pop_front();
    }
  }
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(multicam_imu_calib::DetectionDraw)
